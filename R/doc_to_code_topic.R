...
topic_prior <- tidy_topics_docs %>% 
  filter(Level == 0) %>% 
  group_by(topic) %>% 
  summarise(prior = mean(p))
...
codes_topic <- codes_full %>% 
  ...
  left_join(tidy_topics %>% 
              filter(Level == 0) %>% 
              left_join(topic_prior, by = "topic")
              ) %>% 
  drop_na() %>% 
  group_by(topic, Code) %>% 
  mutate(count = n()) %>% 
  ...
  mutate(p_code_norm = (p_code/prior)/sum(p_code/prior)) %>% 
  select(Code, topic, p_code_norm) %>% 
  drop_na()

matches <- tidy_topics_docs %>% filter(Level == 0) %>% 
  filter(document %in% data$`Report number`) %>% 
  left_join(topic_prior, by = "topic") %>% 
  drop_na() %>% 
  mutate(p_norm = (p/prior)/sum(p/prior)) %>% 
  #filter(doc_ID < 10) %>% # TODO remove this line
  select(document, topic, p,p_norm) %>% 
  inner_join(codes_topic, by = "topic") %>% 
  arrange(Code, topic) %>% 
  left_join(topic_prior, by = "topic") %>% 
  group_by(document, Code) %>% 
  summarise(similarity = sum(p_norm*p_code_norm)) %>% 
