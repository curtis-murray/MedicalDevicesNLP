...
p_w_tw_all_path <- list.files(path = "data/hSBM", pattern=glob2rx("*p_w_tw*.csv"), full.names = T)
p_tw_d_all_path <- list.files(path = "data/hSBM", pattern=glob2rx("*p_tw_d*.csv"), full.names = T)

words_all_all_path <- list.files(path = "data/hSBM", pattern = "words_all*", full.names = T)

# Get the full vocab
# Vocab <- read_csv(words_all_all_path[str_detect(words_all_all_path, "Full")]) %>% 
# 	select(word = `0`)

Vocab <- read_csv("data/hSBM/train.csv") %>% 
...
#write_csv(Vocab, "data/hSBM/Sampling_Problem/Vocab/Vocab.csv")
...
write_csv(tidy_topics, "data/hSBM/Clean/tidy_topics.csv")
...
write_csv(tidy_topic_docs, "data/hSBM/Clean/tidy_topics_docs.csv")
