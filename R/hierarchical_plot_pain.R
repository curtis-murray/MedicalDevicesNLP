...
clean_posts <- read_csv("data/hSBM/train.csv")
...
tidy_topics_all <- read_csv("data/hSBM/Clean/tidy_topics.csv") %>% 
  group_by(Level) %>% 
  nest() %>% 
  ungroup() %>% 
...
tidy_topic_docs_all <- read_csv("data/hSBM/Clean/tidy_topics_docs.csv") %>% 
  group_by(Level) %>% 
  nest() %>% 
  ungroup() %>% 
  transmute(level = Level, posts = data)
...
save(list = "my_graph", file = "data/hSBM/Clean/graph.rda")
...
saveWidget(visIgraph(my_graph,
           ...
           file = paste("Figures/topic_network_alt.html",sep=""))
