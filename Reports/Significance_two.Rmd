...
mesh_filtered %>% 
  ...
  left_join(tidy_topics_docs %>% 
              filter(Level == 1, topic == pain_topic_1),
            by = c("Report number" = "document")) %>% 
  select(hernia, pelvic, p, `Event description`,`Report date`,`Report number`) %>% 
  mutate(pain = ifelse(p >= 0.3, "pain threshold", "no pain threshold")) %>% 
  mutate(pain = ifelse(is.na(p), "no pain threshold",pain)) %>% 
...
x <- list(prr=list(),
            xbar=list(zero_rate = 0.5),
            xbar=list(zero_rate = 0.5,ts_event=c(Rate="rate"), we_rule=2),
            poisson_rare=list(p_rate=0.5), #0.3
          bcpnn = list())
...
```{r}
 df %>% 
  ...
  #mutate(Date = floor_date(`Report date`, unit = unit)) %>%
  mutate(type = ifelse(pelvic, "pelvic", "hernia")) %>% 
  select(ReportID = `Report number`,Date = `Report date`, type, `Event description`,) %>% 
  mutate(pain = str_detect(`Event description`, "(?i)pain")) %>% 
  select(-`Event description`) %>% 
  write_csv("data/pelvic_hernia_pain.csv")
```
...
mesh_pain %>% 
  ...
  mutate(type = ifelse(pelvic, "pelvic","hernia"),
         pain = ifelse(pain, "pain","no pain")) %>% 
  select(Date = `Report date`,type,pain,count) %>% 
  pivot_wider(values_from = count, names_from = pain)


mesh_pain %>% 
  select(Report_ID = `Report number`, Date, pelvic, pain)
...
pain_table <- mesh_pain %>% 
  ...
  group_by(Date, type) %>% 
  nest() %>% 
  pivot_wider(names_from = pain, values_from = count) %>% 
  as.data.frame() %>% 
  arrange(Date) %>% 
  bind_rows(
    tibble(Date = seq.Date(from = min(mesh_pain$Date), to = max(mesh_pain$Date), by = unit)) %>% 
...
```{r}
results <- run_algos(pain_table, algos)
...
mesh_pain <- mesh_filtered %>% 
  ...
  left_join(tidy_topics_docs %>% 
              filter(Level == 1, topic == pain_topic_1),
            by = c("Report number" = "document")) %>% 
  select(`Report number`, hernia, pelvic, p, `Event description`, Date) %>% 
...
pain_table <- mesh_pain %>% 
  ungroup() %>% 
  mutate(type = ifelse(pelvic, "pelvic","hernia")) %>% 
  select(Report_ID = `Report number`, Date,type,pain)
