---
title: "Codes from Adverse Event Reports"
...
data <- data %>% 
  ...
  filter(str_detect(`GMDN term`, "lens"))
...
best <- matches %>% 
  ...
  select(`Event description`, Code, `Code description` = def_full)

best %>% 
  datatable()
```

# Pelvic and hernia mesh
...
data <- data %>%
  mutate(`GMDN term` = str_to_lower(`GMDN term`)) %>%
  filter(str_detect(`GMDN term`, "glucose"))
