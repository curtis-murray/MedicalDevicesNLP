{
  ...
  sidebar <- dashboardSidebar(
    sidebarMenu(
      ...
      textInput("focus_term_2", "More specific term:", value =  "hernia", width = "80%"),
  ...
  server <- function(input, output) {
    ...
    output$wc_from_input <- renderPlot({
      ...
      ggsave(p, filename = paste(input$focus_word, "_", input$topic_level, ".pdf"), width = 6, height = 6)
    ...
    output$topic_table <- renderDataTable(
      docs_metadata %>% 
        ...
        filter(str_detect(`GMDN term`, input$focus_term) & str_detect(content, paste("(?i)",input$focus_term_2, sep = ""))) %>% 
    ...
    output$topic_density <- renderPlot({
      ...
      if(input$topic_level == -1){
        plot_data <- data %>% 
          mutate(`GMDN term` = str_squish(str_to_lower(`GMDN term`))) %>%
          filter(str_detect(`GMDN term`, input$focus_term)) %>% 
          mutate(p = str_count(`Event description`,paste("(?i)",input$focus_word,sep = ""))/(str_count(`Event description`," ")+1)) %>% 
